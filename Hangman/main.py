    ...
    def guess_letter(self):
        ...
        if guess == "":
            messagebox.showwarning(
                title="Empty Space Found!", message="Please enter the character!"
            )
            return

        if guess in self.guessed_letters:
            self.message_label.config(
                text=f"You've already guessed '{guess}'. Try another letter."
            )
        ...
        if guess in self.any_letter:
            ...
            self.update_stage()
            self.message_label.config(
                text=f"Oops! '{guess}' is not in the word. You lost a life."
            )

            if self.count == len(stages) - 1:
                self.message_label.config(
                    text=f"Game Over! The word was '{self.any_letter}'."
                )
                messagebox.showinfo(
